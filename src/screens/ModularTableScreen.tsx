...
export default function ModularTableScreen() {
    ...
    return (
        <SafeAreaView className="flex-1 bg-slate-950">
            <ScreenHeader title={t('tools.modular_table.title')} />
                ...
                <View className="p-6">
                    <View className="bg-sky-500/5 p-5 rounded-2xl border border-sky-500/10 mb-6">
                        <View className="flex-row items-center mb-3">
                            ...
                            <Text className="text-sky-400 font-bold ml-2 text-xs uppercase">{t('visualizers.modular_table.looking_at')}</Text>
                        </View>
                        <Text className="text-slate-400 text-xs leading-5">
                            <Trans
                                i18nKey="visualizers.modular_table.table_desc"
                                components={{
                    ...
                    <View className="mb-8">
                        <View className="flex-row justify-between items-center mb-4">
                            <Text className="text-slate-400 font-medium">{t('visualizers.modular_table.mod_label')}</Text>
                    ...
                    <View className="mt-8 space-y-4">
                        <View className="bg-slate-900 p-5 rounded-2xl border border-slate-800">
                            <Text className="text-slate-400 text-xs uppercase tracking-widest font-bold mb-3">{t('visualizers.modular_table.interactive_guide')}</Text>
                            <Text className="text-slate-300 text-sm leading-6">
                                {t('visualizers.modular_table.guide_desc')}
                            </Text>
                        </View>
                        ...
                        {highlightResidue !== null && (
                            <Animated.View entering={FadeIn} className="bg-sky-500/10 p-5 rounded-2xl border border-sky-500/20">
                                <View className="flex-row items-center justify-between">
                                    <Text className="text-sky-400 font-bold">{t('visualizers.modular_table.highlight_label', { residue: highlightResidue })}</Text>
                                    <TouchableOpacity onPress={() => setHighlightResidue(null)}>
                                        <Text className="text-slate-500 text-xs font-bold">{t('visualizers.modular_table.clear')}</Text>
                                    </TouchableOpacity>
                                </View>
                            </Animated.View>
                        )}
