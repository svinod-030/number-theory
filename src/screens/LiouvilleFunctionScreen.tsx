...
export default function LiouvilleFunctionScreen() {
    const { t } = useTranslation();
    const [n, setN] = useState('');
    const [result, setResult] = useState<number | null>(null);
    const [factors, setFactors] = useState<number[]>([]);

    const calculateLiouville = () => {
        Keyboard.dismiss();
        try {
    ...
    return (
        <SafeAreaView className="flex-1 bg-slate-950">
            <ScreenHeader title={t('tools.liouville_function.title')} />
            ...
            >
                <MathCard
                    index={0}
                    title={t('visualizers.liouville_function.simple_terms_title')}
                >
                    <View className="bg-emerald-500/5 p-5 rounded-2xl border border-emerald-500/10 mb-4">
                        <View className="flex-row items-center mb-3">
                            ...
                            <Text className="text-emerald-400 font-bold ml-2 text-xs uppercase">{t('visualizers.liouville_function.simple_terms_title')}</Text>
                        </View>
                        <Text className="text-slate-400 text-xs leading-5">
                            <Trans
                                i18nKey="visualizers.liouville_function.simple_terms_desc"
                                components={{
                ...
                <MathCard
                    index={1}
                    description={t('visualizers.liouville_function.description')}
                >
                    <View className="space-y-4">
                        <View>
                            <Text className="text-slate-400 text-xs font-bold uppercase mb-2 ml-1">
                                {t('visualizers.liouville_function.input_n')}
                            </Text>
                            <TextInput
                        ...
                        >
                            ...
                            <Text className={`font-black ml-2 uppercase tracking-widest text-sm ${
                                ...
                                {t('visualizers.liouville_function.calculate')}
                            </Text>
                        </TouchableOpacity>
                    </View>
                </MathCard>
                            ...
                            <View className={`p-6 rounded-2xl border ${
                                ...
                                <View className="items-center mb-6">
                                    <View className={`w-16 h-16 rounded-full items-center justify-center mb-4 ${
                                        result === 1 ? 'bg-emerald-500/20' : 'bg-sky-500/20'
                                    }`}>
                                        <Text className={`text-3xl font-black ${
                                            result === 1 ? 'text-emerald-400' : 'text-sky-400'
                                        }`}>
                                            {result}
                                        </Text>
                                    </View>
                                    <Text className="text-white text-xl font-black text-center mb-1">
                                        {result === 1 
                                            ? t('visualizers.liouville_function.result_one', { n }) 
                                            : t('visualizers.liouville_function.result_neg_one', { n })
                                        }
                                    </Text>
                                </View>
                                ...
                                {factors.length > 0 && (
                                    <View className="bg-slate-900 border border-slate-800 rounded-xl p-4">
                                        <Text className="text-slate-400 text-xs font-bold uppercase mb-2 text-center">
                                            {t('visualizers.liouville_function.prime_factors', { count: factors.length })}
                                        </Text>
                ...
                <Animated.View entering={FadeInUp.delay(200)} className="mt-4">
                    <MathCard index={3} title={t('visualizers.liouville_function.polya_conjecture')}>
                        <View className="bg-purple-500/5 p-5 rounded-2xl border border-purple-500/10">
                            <View className="flex-row items-center mb-3">
                                <Ionicons name="telescope-outline" size={18} color="#c084fc" />
                                <Text className="text-purple-400 font-bold ml-2 text-xs uppercase">{t('visualizers.liouville_function.polya_conjecture')}</Text>
                            </View>
                            <Text className="text-slate-400 text-xs leading-5">
                                {t('visualizers.liouville_function.polya_desc')}
                            </Text>
                        </View>
                    </MathCard>
                </Animated.View>
