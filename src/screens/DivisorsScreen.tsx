    ...
    return (
        <SafeAreaView className="flex-1 bg-slate-950">
            <View className="px-6 py-4 flex-row items-center justify-between border-b border-slate-900">
                ...
                <Text className="text-xl font-bold text-white">{t('visualizers.divisors.title')}</Text>
                <View style={{ width: 24 }} />
            </View>
                ...
                <View className="px-6 pt-6">
                    <View className="bg-emerald-500/5 p-5 rounded-2xl border border-emerald-500/10 mb-6">
                        ...
                        <Text className="text-slate-400 text-xs leading-5">
                            <Trans
                                i18nKey="visualizers.divisors.analogy_desc"
                                components={{
                    ...
                    <View className="flex-row items-center space-x-4 mb-8">
                        <View className="flex-1">
                            <Text className="text-slate-500 text-xs mb-2 uppercase tracking-widest font-bold">{t('visualizers.divisors.input_label')}</Text>
                            <TextInput
                                value={input}
                        ...
                        >
                            <Text className="text-white text-xs font-bold uppercase tracking-tighter opacity-80">{t('visualizers.divisors.type_badge')}</Text>
                            <Text className="text-white font-black text-lg">{t(`visualizers.divisors.${classification.toLowerCase()}_title`)}</Text>
                        </Animated.View>
                    </View>

                    <View className="bg-slate-900 p-6 rounded-3xl border border-slate-800 mb-8 items-center">
                        <Text className="text-slate-500 text-xs mb-2 uppercase tracking-widest font-bold text-center">{t('visualizers.divisors.sum_label')}</Text>
                    ...
                    <Text className="text-slate-500 text-xs mb-4 uppercase tracking-widest font-bold">{t('visualizers.divisors.all_divisors_label', { n })}</Text>
                    <View className="flex-row flex-wrap mb-10">
                        {properDivisors.concat(n).map((d, i) => {
                            ...
                            return (
                                ...
                                >
                                    <Text className="text-slate-500 text-[10px] uppercase font-bold mb-1">{isProper ? t('visualizers.divisors.proper_label') : t('visualizers.divisors.self_label')}</Text>
                    ...
                    <View className="bg-slate-900/50 p-6 rounded-3xl border border-slate-800/50 mb-12">
                        <View className="flex-row items-center mb-4">
                            ...
                            <Text className="text-sky-400 font-bold ml-2">{t('visualizers.divisors.definition_title')}</Text>
                        </View>
                        <View className="space-y-4">
                            <View>
                                <Text className="text-white font-bold mb-1">{t('visualizers.divisors.perfect_title')}</Text>
                                <Text className="text-slate-400 text-sm leading-5">{t('visualizers.divisors.perfect_desc')}</Text>
                            </View>
                            <View className="border-t border-slate-800 pt-4">
                                <Text className="text-white font-bold mb-1">{t('visualizers.divisors.abundant_title')}</Text>
                                <Text className="text-slate-400 text-sm leading-5">{t('visualizers.divisors.abundant_desc')}</Text>
                            </View>
                            <View className="border-t border-slate-800 pt-4">
                                <Text className="text-white font-bold mb-1">{t('visualizers.divisors.deficient_title')}</Text>
                                <Text className="text-slate-400 text-sm leading-5">{t('visualizers.divisors.deficient_desc')}</Text>
                            </View>
                        </View>
                    </View>
                </View>
